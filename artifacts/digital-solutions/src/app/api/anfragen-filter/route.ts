...
export async function POST(req: NextRequest) {
  try {
    const { answers, answerIndexes, lang } = await req.json();
...
Language: ${lang === "en" ? "English" : lang === "es" ? "Spanish" : "German"}`,
    ...
    return NextResponse.json({ score, summary: result.summary, nextStep: result.nextStep });
  } catch (error) {
    console.error("Anfragen filter API error:", error);
