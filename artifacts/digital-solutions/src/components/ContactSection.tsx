...
export default function ContactSection() {
  ...
  useEffect(() => {
    function prefillContact(event: Event) {
      const subject = (event as CustomEvent<string>).detail;
      if (subject) setValue("message", subject, { shouldDirty: true });
    }

    window.addEventListener("contact-prefill", prefillContact);
    return () => window.removeEventListener("contact-prefill", prefillContact);
  }, [setValue]);
  ...
  return (
    <section id="contact" ref={ref} className="py-24 px-4 bg-black relative" aria-labelledby="contact-heading">
      <span id="kontakt" className="absolute top-0" aria-hidden="true" />
      ...
      <div className="max-w-xl mx-auto relative z-10">
        ...
        <motion.p
          ...
          className="text-center text-emerald-300 font-semibold text-xl mb-4 tracking-wide"
        >
          {t.contact.cta}
        </motion.p>

        <p className="text-center text-slate-400 text-lg mb-10">{t.contact.sub}</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              <div className="space-y-6">
                <div>
                  ...
                  {errors.name && <p className="mt-2 text-sm text-red-400 pl-1">{errors.name.message}</p>}
                </div>
                <div>
                  ...
                  {errors.email && <p className="mt-2 text-sm text-red-400 pl-1">{errors.email.message}</p>}
                </div>
                <div>
                  ...
                  {errors.message && <p className="mt-2 text-sm text-red-400 pl-1">{errors.message.message}</p>}
                </div>
                <div className="flex items-start gap-4 p-4 rounded-xl bg-black/30 border border-white/5">
                  ...
                  <div>
                    ...
                    {errors.gdpr && <p className="mt-1.5 text-sm text-red-400">{errors.gdpr.message}</p>}
                  </div>
                </div>
                {status === "error" && (
