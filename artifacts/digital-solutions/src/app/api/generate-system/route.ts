...
export async function POST(req: NextRequest) {
  try {
    const { profession, problem, goal, lang = "de" } = await req.json();
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: buildUserMessage(profession, problem, goal, lang),
        },
      ],
    });
    ...
    console.error("generate-system API error:", error);
    return NextResponse.json(
